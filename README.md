...
| **Quicksilver confirm path** | Scoreboard / observation vision is `qwen3.7-flash` on the same Quicksilver API + clutchbot key as ClutchBot chat. Not Gemini. `deepseek-v4-flash` is chat-only (JPEG crop is 400 `model_not_found`). JPEG crop in, JSON scorebug out |
