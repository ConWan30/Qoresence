...
def main():
    ...
    for s in SCENARIOS:
        ...
        rows.append(
            {
                "scenario": s["name"],
                "win_prob": round(r["win_prob"], 3),
                "ep": round(r["expected_points"], 2),
                "yds_to_opp": r["yds_to_opp"],
            }
        )
        print(
            f"{s['name']:20} wp={r['win_prob']:.3f} ep={r['expected_points']:.1f} yds={r['yds_to_opp']}"
        )
    ...
    try:
        ...
        print(
            "clip demo:",
            round(m.predict({"wp_swing": 0.15, "red_zone": 1, "close_game": 1, "apm": 0.7}), 3),
        )
