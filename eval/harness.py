...
@dataclass
class BenchResult:
    total: int; cat_acc: float; title_acc: float
    football_f1: float; latency_p50_ms: float; latency_p95_ms: float
...
def evaluate(predict_fn, dataset_path: str | Path = "eval/scoreboard_bench.json") -> BenchResult:
    ...
    for s in samples:
        ...
        if pc and pc==exp_cat: cat_ok+=1
        if pt and exp_title and pt==exp_title: title_ok+=1
        # football fields
        if exp_cat=="football":
            fb_total+=1
            if isinstance(pred, dict):
                ...
                if hs_ok and aw_ok: fb_ok+=1
    lat.sort()
    n=len(samples)
    p50=lat[n//2] if lat else 0
    p95=lat[int(n*0.95)] if lat else 0
    return BenchResult(n, cat_ok/max(1,n), title_ok/max(1,n), fb_ok/max(1,fb_total), p50, p95)

def mock_predict(sample: dict) -> dict:
    exp=sample.get("expected", sample)
    return {"game_category": exp.get("game_category","football"), "game_title": exp.get("game_title","NCAA Football 27"), "home_score": exp.get("home_score"), "away_score": exp.get("away_score")}
