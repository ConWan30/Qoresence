...
export type DeckIngest = {
  ...
  homeTeam: string;
  awayTeam: string;
  /** True when HOME is the left scorebug. Madden/NFL/CFB default is away-left. Identity only — must not drive digit placement. */
  homeLeft: boolean;
...
/** Named HDMI paint: left_score+right_score → left→right as cropped. Never homeLeft-swap named digits. Missing left_* → unlabeled home–away (fail-closed). */
export function scorebugPair(ing: {
  ...
  // Fail-closed: do not silently remap named Madden boards via homeLeft.
  return dash === "–" ? `${ing.homeScore}–${ing.awayScore}` : `${ing.homeScore}-${ing.awayScore}`;
}
