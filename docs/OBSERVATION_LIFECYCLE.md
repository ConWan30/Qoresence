...
this host (DualSense on PS5 is Path B default). Outcome is null. Structured
`uncertainty_channels` on each record separate capture, OCR, input, clock,
boundary, and outcome trust — see `docs/ASTRA_SLICE_D_UNCERTAINTY.md`.
Football adapter phases: `docs/ASTRA_SLICE_C_FOOTBALL_ADAPTER.md`.
...
`python -m pytest tests/test_observation_lifecycle.py tests/test_observation_event_replay.py tests/test_observation_football_adapter.py tests/test_observation_uncertainty.py tests/test_observation_eval_harness.py tests/test_observation_claim_ledger.py tests/test_deadlock_regression.py`
