...
ClutchBot **chat** default on Quicksilver Pro is now `muse-spark-1.3`. Confirm / scoreboard VLM stays `gemini-3.8-flash`. DualSense stays on the PS5.

| Surface | Change |
|---------|--------|
| `qoresence/agents/llm_client.py` | `DEFAULT_MODEL = "muse-spark-1.3"`. `DEFAULT_VISION_MODEL` remains `gemini-3.8-flash`. `DEFAULT_BASE_URL` remains `https://api.quicksilverpro.io/v1`. Module docstring/comments updated. |
