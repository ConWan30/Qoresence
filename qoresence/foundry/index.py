...
CONFIRM_KINDS = frozenset({"confirm","confirm_chat","confirm_clip","confirm_score","prediction_resolve"})
...
def scan_clips(clips_dir=None):
    ...
    try:
        ...
        for mp4 in sorted(d.glob("*.mp4"), key=lambda x: x.stat().st_mtime, reverse=True):
                ...
                if isinstance(bt.get("buttons_summary"), dict):
                    ...
                    for e in bt.get("events", []):
                        if isinstance(e, dict) and e.get("name"):
                            buttons_summary[str(e["name"])] = buttons_summary.get(str(e["name"]),0)+1
            ...
            try:
                st=mp4.stat(); mtime=float(st.st_mtime); size=int(st.st_size)
            except Exception:
                mtime=0.0; size=0
            out.append({"clip": str(mp4).replace("\\","/"), "stem": stem, "mtime": mtime, "size_bytes": size, "chapters": chapters if isinstance(chapters, list) else [], "buttons_summary": buttons_summary, "why": why, "graph_summary": gs})
    except Exception as e:
...
def _clip_search_text(chapters, buttons_summary, why, gs):
    ...
    for c in chapters[:12]:
        parts.append(str(c.get("label") or "")); parts.append(str(c.get("kind") or "")); parts.append(str(c.get("path") or ""))
    if buttons_summary:
        parts.extend(list(buttons_summary.keys()))
    if why:
        parts.append(str(why.get("line") or "")); parts.append(str(why.get("phase") or ""))
    if gs:
...
def _score_clip(qtok, clip, now_s):
    ...
    why=clip.get("why"); gs=clip.get("graph_summary")
    ...
    if overlap==0 and qtok:
        return 0.0, None
    best=None; best_bonus=0.0
    for c in chapters:
        k=str(c.get("kind") or ""); b=0.0
    ...
    return score, best
def search_clips(query="", limit=8, since_clock_ns=0, kinds="", coupling_min=0.0, drive_id=None, clips_dir=None):
    ...
    clips=scan_clips(clips_dir); now_s=time.time()
    scored=[]
    for clip in clips:
        if drive_id:
            gs=clip.get("graph_summary") or {}; cd=str(gs.get("drive_id") or "")
            if cd!=drive_id and drive_id not in clip.get("clip","") and drive_id not in clip.get("stem",""):
                continue
        if coupling_min>0:
            cs=[]
            for c in clip.get("chapters") or []:
                try:
                    if c.get("coupling") is not None: cs.append(float(c["coupling"]))
                except Exception: continue
            if cs and max(cs) < coupling_min: continue
        if want_kinds:
            ch_kinds={str(c.get("kind") or "") for c in (clip.get("chapters") or [])}
            if not (ch_kinds & want_kinds): continue
    ...
    for score, clip, best in scored[:limit]:
        chapters=clip.get("chapters") or []; t_s=None; kind=""; label=""
        if best: t_s=best.get("t_s"); kind=str(best.get("kind") or ""); label=str(best.get("label") or "")
        hits.append({"clip": clip.get("clip"), "t_s": t_s, "label": label, "kind": kind, "buttons_summary": clip.get("buttons_summary") or {}, "drive_id": (clip.get("graph_summary") or {}).get("drive_id") if isinstance(clip.get("graph_summary"), dict) else None, "graph": clip.get("graph_summary"), "why": clip.get("why"), "score": round(float(score),3), "chapters": chapters[:3]})
    if not hits:
        try:
            from qoresence.agents.session_timeline import get_session_timeline
            tl=get_session_timeline(); events=tl.recent(80) if hasattr(tl,"recent") else []
            filt=[]
            for ev in events:
                d=ev.to_dict() if hasattr(ev,"to_dict") else dict(ev) if isinstance(ev, dict) else {}
                blob=f"{d.get('message','')} {d.get('reason','')} {d.get('kind','')} {d.get('path','')}".lower()
                bt2=_tokenize(blob)
                if not qtok or (qtok & bt2): filt.append(d)
            filt=filt[-limit:]
            for d in filt:
                if want_kinds and str(d.get("kind") or "") not in want_kinds: continue
                if coupling_min and d.get("coupling") is not None:
                    try:
                        if float(d["coupling"]) < coupling_min: continue
                    except Exception: pass
                if drive_id and str(d.get("drive_id") or "") != drive_id: continue
                if since_clock_ns and int(d.get("clock_ns") or 0) <= int(since_clock_ns): continue
                hits.append({"clip": None, "label": str(d.get("message") or d.get("reason") or d.get("kind") or ""), "kind": str(d.get("kind") or ""), "buttons_summary": {}, "drive_id": d.get("drive_id"), "graph": None, "why": None, "score": 0.5, "chapters": [], "timeline_event": d})
            hits=hits[:limit]
        except Exception as e: log.debug("timeline fallback: %s", e)
...
def get_drive_graph(drive_id=None, include_nodes=True, max_nodes=40):
    include_nodes=bool(include_nodes); max_nodes=max(1, min(200, int(max_nodes)))
    try:
        from qoresence.agents.drive_graph import DriveGraph, active_drive_graph
        from qoresence.agents.session_timeline import get_session_timeline
        tl=get_session_timeline(); g=None; did=(drive_id or "").strip()
        if not did or did.lower()=="active":
            g=active_drive_graph(tl)
        else:
            drives=tl.drives() if hasattr(tl,"drives") else []; target=None
            for d in drives:
                try:
                    td=d.to_dict() if hasattr(d,"to_dict") else {}
                    if getattr(d,"drive_id","")==did or td.get("drive_id")==did: target=d; break
                except Exception: continue
            if target is not None: g=DriveGraph.from_timeline_drive(tl, target)
            else:
                events=[e for e in (tl.recent(200) if hasattr(tl,"recent") else []) if getattr(e,"drive_id",None)==did or (e.to_dict().get("drive_id") if hasattr(e,"to_dict") else None)==did]
                if events: g=DriveGraph.from_events(did, events)
        if g is None:
            return {"ok": False, "error": "no_drive", "hint": "no active drive; run --play to create drives"}
        d=g.to_dict(include_nodes=include_nodes)
        if include_nodes and len(d.get("nodes",[]))>max_nodes: d["nodes"]=d["nodes"][:max_nodes]; d["trimmed"]=True
        d["ok"]=True
        if "why_line" not in d:
            try: d["why_line"]=g.why_line()
            except Exception: pass
        return d
    except Exception as e:
