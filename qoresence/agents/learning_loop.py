...
class ClipWorthinessTrainer:
    ...
    def _featurize(self, s: LearningSample) -> list[float]:
        ...
        is_rz = (
            1.0
            if "opp" in pos.lower()
            and any(f"opp {i}" in pos.lower() for i in range(1, 21))
            or "opp 1" in pos.lower()
            else 0.0
        )
        ...
        apm = float(
            sit.get("controller", {}).get("apm_5s", 0)
            if isinstance(sit.get("controller"), dict)
            else 0
        )
        apm_n = min(1.0, apm / 120.0)
        return [float(s.wp_swing), is_rz, close, apm_n]

    def train_from_logger(
        self, logger: LearningLogger, lr: float = 0.5, iters: int = 300, l2: float = 0.01
    ) -> dict:
