...
class TrioRetinaConfig:
    ...
    _resolved_wasm_path: Path | None = field(default=None, init=False, repr=False)
    _cached_block_number: int | None = field(default=None, init=False, repr=False)
    _block_cache_ts: float = field(default=0.0, init=False, repr=False)

    def resolve_wasm_path(self, base_dir: Path | None = None) -> Path:
