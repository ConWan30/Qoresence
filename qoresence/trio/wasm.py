...
class WasmtimeRunner:
    ...
    def __init__(self, config: TrioRetinaConfig):
        self.config = config
        self._wasm_path: Optional[Path] = None
        ...
        try:
            ...
            if self.config.log_validation_results:
                if result.ok or not self.config.log_failures_only:
                    log.info(f"trio-retina validation: exit={result.exit_code} ({result.error_description}) in {duration_ms:.1f}ms")
                else:
