...
from qoresence.core import SessionAuthority, SessionIdentity, RetinaEventBus
...
def compute_node_id(device_id_hex: str, first_session_id: str, prefix: str = "QORTROLLER-NODE-v0") -> str:
    """
...
def try_real_pq_commitment(
    ...
    zk_artifacts_dir: Optional[Path] = None,
) -> Optional[str]:
    """
    ...
    if zk_artifacts_dir is None:
        ...
        if env_path:
            ...
            candidates = [
                Path(__file__).parent.parent.parent / "vapi-pebble-prototype" / "bridge" / "zk_artifacts",
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            proof_bytes = bytes.fromhex(
                proof["pi_a"][0][2:] + proof["pi_a"][1][2:] +
                proof["pi_b"][0][0][2:] + proof["pi_b"][0][1][2:] +
                proof["pi_b"][1][0][2:] + proof["pi_b"][1][1][2:] +
                proof["pi_c"][0][2:] + proof["pi_c"][1][2:]
            )
...
def mock_signature(payload_hash: str, device_key: Optional[bytes] = None) -> str:
    """
...
def build_evm_log_payload(
    ...
    visual_oracle_root: Optional[str] = None,
    posp_root: Optional[str] = None,
    first_session_id: Optional[str] = None,
    device_key: Optional[bytes] = None,
    # Real PQ commitment inputs (when using ZKSepProof)
    biometric_snapshot_hash: Optional[str] = None,
    claimed_player_id: Optional[int] = None,
    feature_commitment: Optional[str] = None,
) -> EvmLogPayload:
    """
    ...
    if config.pq_commitment_source == "real" and biometric_snapshot_hash and claimed_player_id is not None and feature_commitment:
